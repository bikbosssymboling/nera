...
export default function SetPositionModal({
  ...
  const [originalData, setOriginalData] = useState<typeof positionAndKPI | null>(null);
  const [originalKPI, setOriginalKPI] = useState<string | null>(null);
  const [positionAndKPI, setPositionAndKPI] = useState([
    { id: "pos1", position: "Position 1", dailyCompensation: "1,000", isDeleted: false, isModified: false },
    { id: "pos2", position: "Position 2", dailyCompensation: "2,000", isDeleted: false, isModified: false },
    { id: "pos3", position: "Position 3", dailyCompensation: "3,000", isDeleted: false, isModified: false },
    { id: "pos4", position: "Position 4", dailyCompensation: "4,000", isDeleted: false, isModified: false }
  ]);

  const positions = ["--Select Position--", "Position 1", "Position 2", "Position 3", "Position 4"];
  ...
  const handleAddRow = () => {
    ...
    setPositionAndKPI([
      ...positionAndKPI,
      { id: `pos${Date.now()}`, position: "--Select Position--", dailyCompensation: "", isDeleted: false, isModified: false }
    ]);
    ...
    setTimeout(() => {
      ...
      if (tableContainer) {
        ...
        const newSelect = document.querySelector(`[data-row-index="${newIndex}"] select`);
  ...
  const handlePositionChange = (index: number, value: string) => {
    const updatedData = [...positionAndKPI];
    updatedData[index] = { ...updatedData[index], position: value, isModified: true };
    setPositionAndKPI(updatedData);
  };

  const handleDeleteRow = (index: number) => {
    if (positionAndKPI[index].position === "--Select Position--" || !positionAndKPI[index].dailyCompensation) {
      const updatedData = positionAndKPI.filter((_, i) => i !== index);
      if (updatedData.length === 0) {
        setPositionAndKPI([{ id: `pos${Date.now()}`, position: "--Select Position--", dailyCompensation: "", isDeleted: false, isModified: false }]);
      } else {
        setPositionAndKPI(updatedData);
      }
      return;
  ...
  const isFormValid = () => {
    return positionAndKPI.every(item => 
      item.position !== "--Select Position--" && 
      item.dailyCompensation !== ""
    );
  };
        ...
        positions: {
          upsert: positionAndKPI
            ...
            .map(item => ({
              ...
              dailyCompensation: parseInt(item.dailyCompensation.replace(/,/g, ''))
            })),
          ...
          <table className="table-auto w-full border-collapse border border-gray-300">
            <thead className="sticky top-0 bg-gray-200 z-10">
              <tr>
                ...
                <th className="border border-gray-300 px-4 py-2">Daily Compensation (THB)</th>
            ...
            <tbody className="relative z-0">
              {positionAndKPI.map((item, index) => (
                <tr 
                  ...
                  className={`tr-highlight ${item.isDeleted ? 'opacity-50 bg-gray-100' : ''}`}
                >
                  ...
                  <td className="border border-gray-300 px-4 py-2">
                    {item.isDeleted ? (
                      <div className="text-gray-500">{item.dailyCompensation} บาท</div>
                    ) : (
                      <div className="relative w-full">
                        <input
                          type="text"
                          value={`${item.dailyCompensation}`}
                          onChange={(e) => handleCompensationChange(index, e.target.value)}
